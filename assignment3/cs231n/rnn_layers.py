...
def rnn_step_backward(dnext_h, cache):
    ...
    db = ((1 - next_h**2) * dnext_h).sum(axis=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        dx[:, t, :], dprev_h, dWxt, dWht, dbt = rnn_step_backward(dh[:, t, :] + dprev_h, cache.pop())
        dWx += dWxt
        dWh += dWht
        db += dbt
    dh0 = dprev_h
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dprev_c = dc * f
    da = np.hstack((dc * g * i * (1 - i), 
                    dc * prev_c * f * (1 - f), 
                    dnext_h * tanh_c * o * (1 - o),
                    dc * i * (1 - g**2)))
    dx = da.dot(Wx.T)
    dprev_h = da.dot(Wh.T)
    dWx = x.T.dot(da)
...
def lstm_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        dx[:, t, :], dprev_h, dprev_c, dWxt, dWht, dbt = lstm_step_backward(dh[:, t, :] + dprev_h, dprev_c, cache.pop())
        dWx += dWxt
        dWh += dWht
        db += dbt
    dh0 = dprev_h
